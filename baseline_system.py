...
class Database:
    ...
    def fetch_reviewers() -> List[Dict]:
        ...
        conn.close()
        return [{"id": r[0], "name": r[1], "email": r[2], "current_workload": r[3], "max_workload": r[4], "conflict_subject": r[5]} for r in rows]
...
class ReviewerManager:
    def get_available_reviewers(self) -> List[Dict]:
        db = Database()
        reviewers = db.fetch_reviewers()
        filtered = self.filter_conflicts(reviewers, "AI")
...
class Reviewer:
    def assign_review(self, submission_id: int, reviewer_id: int):
        pass
    
    def submit_score(self, submission_id: int, reviewer_id: int, score: int, eval_manager):
...
class EvaluationManager:
    ...
    def submit_score(self, score: int, submission_id: int, reviewer_id: int):
...
class NotificationService:
    def notify_acceptance(self, researcher_email: str):
        print(f"Notification: Acceptance sent to {researcher_email}")
    def notify_rejection(self, researcher_email: str):
        print(f"Notification: Rejection sent to {researcher_email}")
    def notify_revision(self, researcher_email: str):
        print(f"Notification: Revision requested sent to {researcher_email}")
